...
const PageList: React.FC = () => {
  ...
  const handleDelete = async (
    ...
    try {
      ...
      await Swal.fire({
        title: 'Success!',
        text: `"${pageTitle}" has been deleted successfully.`,
        icon: 'success',
        confirmButtonColor: '#3085d6',
        confirmButtonText: 'OK',
      });
  ...
  const handleUnpublish = async (pageId: string, pageTitle: string) => {
    const result = await Swal.fire({
      ...
      text: `"${pageTitle}" will no longer be visible to the public.`,
      icon: 'warning',
      showCancelButton: true,
  ...
  const handleBulkAction = async () => {
    ...
    const actionText = bulkAction === 'delete' ? 'delete' : 'publish';
    ...
    try {
      ...
      alert(`Successfully ${actionText}ed ${selectedPages.length} page(s).`);
    } catch (err: any) {
          ...
          <div className='alert alert-info mb-4'>
            <div className='d-flex align-items-center'>
              ...
              <div>
                ...
                system pages. You can edit them but not delete them. Custom
                pages can be fully managed here.
              </div>
            </div>
          </div>
